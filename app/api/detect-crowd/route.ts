...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[API] Forwarding to Python inference server (127.0.0.1:5000)...",
    );
    ...
    try {
      ...
      console.log(
        `[API] ML Server responded with status: ${mlResponse.status}`,
      );
      ...
      console.log(
        `[API] Detection Result: ${mlDetections.count} people, risk: ${mlDetections.riskLevel}`,
      );
      ...
      console.error(
        "[API] Failed to connect to Python server:",
        fetchError.message,
      );
      return NextResponse.json(
        {
