...
def main(_argv):
    ...
    # otherwise load standard tensorflow saved model
        ...
        # Check if weights path exists as SavedModel
        if os.path.exists(FLAGS.weights) and os.path.isdir(FLAGS.weights):
            saved_model_loaded = tf.saved_model.load(FLAGS.weights, tags=[tag_constants.SERVING])
            infer = saved_model_loaded.signatures['serving_default']
        else:
            # Try to create model from Darknet weights
            print(f"Creating model from Darknet weights: {FLAGS.weights}")
            try:
                # Create model
                STRIDES, ANCHORS, NUM_CLASS, XYSCALE = utils.load_config(FLAGS)
                ...
                model = tf.keras.Model(input_layer, [pred_bbox, pred_prob])
                ...
                infer = infer_func
                print("Model created successfully from Darknet weights!")

            except Exception as e:
                print(f"Error creating model from Darknet weights: {e}")
                print("Please convert weights to SavedModel format first using save_model.py")
                return
